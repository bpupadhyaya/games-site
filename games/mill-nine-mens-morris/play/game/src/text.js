...
export const ABOUT = [
  { title: 'Where it is played', lines: ['Nine Men\'s Morris is one of the most widely played traditional board games in Europe. It is called Muhle ("mill") in German, Moulin in French, and Morris or Merels in English, after the rows of three that give the game its name.'] },
  { title: 'Carved in stone and wood', lines: ['Boards are scratched into cloister seats and steps in medieval abbeys and cathedrals, including Westminster Abbey and the cathedrals of Canterbury, Gloucester, Norwich and Salisbury.', 'A board was also found in the Gokstad ship burial in Norway, from about AD 900.'] },
  { title: 'On the green and in the tavern', lines: ['The game needs only a pattern of lines and eighteen small markers. In England the board was sometimes cut into turf on a village green: Shakespeare has a character remark that "the nine men\'s morris is filled up with mud".'] },
  { title: 'A game with a known answer', lines: ['In the 1990s the computer scientist Ralph Gasser searched every position of the game and showed that with perfect play by both sides it ends in a draw.', 'People still win it every day, because nobody plays perfectly.'] },
];
export const HOW = [
  { title: 'Goal', lines: ['Reduce the enemy to two men, or leave them with no legal move.'] },
  { title: '1. Placing', lines: ['Light starts. TAP an empty point to place a man.', 'Each side places nine, one turn at a time.'] },
  { title: '2. Sliding', lines: ['When all men are placed, TAP one of your men, then TAP a glowing point next to it. You can also DRAG the man there.', 'Men slide one step along a line.'] },
  { title: 'Mills', lines: ['Three of your men in a row on one line is a mill. Each time you make one, TAP a glowing red enemy man to take it.', 'Men in a mill are safe unless all the enemy men are in mills.'] },
  ...
  { title: 'Buttons', lines: ['Undo takes back your last move. Hint shows a good move and why.', 'On a keyboard: arrow keys move, Space or Enter is a TAP, U is Undo, H is Hint, Esc is Menu.'] },
];
...
export const RULES = [
  ...
  },
  {
    title: 'Flying',
    lines: [
      'Flying: the instant a side is left with exactly three men on the board and none left in hand, that side starts flying - each turn it may move one man to ANY empty point on the board, not only an adjacent one.',
      'It keeps this privilege for as long as it stays at exactly three men (and can lose it again if it is reduced further, at which point the very next mill against it usually ends the game - see Winning).',
    ],
  },
  {
    title: 'Mills',
    lines: [
      ...
      'The instant you form a mill, you immediately remove one enemy man from the board (see the Captures page for which one). A man removed this way is gone for the rest of the game - it does not return to hand.',
    ],
  },
  {
    title: 'Remaking a mill',
    lines: [
  ...
  {
    title: 'Repetition and the 100-ply rule',
    lines: [
