...
export function createGame(env) {
  ...
  const state = {
    ...
    // Auto Play think-time: index into THINK_STEPS, never a raw float, same pattern as textScaleIdx.
    autoThinkIdx: 1,
  ...
  const savePrefs = () => storage.set('prefs', { level: state.level, sound: state.sound, calm: state.calm, big: state.big, target: state.target, back: state.back, textScaleIdx: state.textScaleIdx, autoThinkIdx: state.autoThinkIdx });
  ...
  // (thock/flick/chime all call it), so gating it here silences the whole mode at once.
  const tone = (o) => { if (state.sound && !(state.tb && state.tb.mode === 'auto')) audio.tone(o); };
  ...
  function newTable(mode, H, extra = {}) {
    ...
    return Object.assign({ mode, H, scores: [0, 0], target: state.target, dealer: H.dealer, handNo: 1, level: state.level, sel: -1, drag: null, flights: [], collect: null, deal: null, wait: 0, msg: null, hint: null, hintsLeft: HINTS_PER_MATCH, undo: [], summary: null, over: null, blocked: false, leaving: false, lessonMsg: '', shake: null, redealIn: 0, coachKey: '', kb: -1,
      // `auto` (pre-existing): true means nobody manually controls seat 0 right now - already used
      // by the puzzle "show me the solution" replay; Auto Play mode reuses the same flag so every
  ...
  function beginHand(dealer) {
    ...
    say(tb.mode === 'auto' ? `Hand ${tb.handNo}. ${first} bids first.` : `Hand ${tb.handNo}. ${first === 'You' ? 'You bid first.' : first + ' bids first.'}`, 3);
    saveMatch();
  }
  function startMatch() {
  ...
  // Auto Play ("Watch & Learn"): a whole match, every seat (including seat 0) driven by the same
  // AI choosers used for the computer opponents (ai.js chooseBid/chooseTrump/pickCard - no new
  // move-picker), through THINK -> REVEAL -> ACT (autoTick, below). Hands are dealt for real
  // (rules.js `deal()`, unchanged) and shown open (view.js), since with all four seats
  // computer-played there is no hidden information left to protect and the viewer needs to see
  // every seat's hand to compare its own guess against. Never touches the real save/stats/demo
  // counter - see the `tb.mode !== 'auto'` guards in handEnd()/saveMatch() and startAutoMatch()
  // itself never reading or writing `state.demoMatches`.
  function startAutoMatch() {
    const dealer = rng.int(4);
    state.tb = newTable('auto', deal(rng, dealer), { auto: true });
    state.scene = 'table'; beginHand(dealer);
  }
  function resume() {
  ...
  function doTrump(s) {
    const tb = state.tb, H = tb.H;
    if (H.declarer === 0 && tb.mode !== 'auto') snapshot();
  ...
  function handEnd() {
    ...
    tb.scores[0] += r.delta[0]; tb.scores[1] += r.delta[1];
    if (tb.mode !== 'auto') state.stats.hands++; // Auto Play never touches the real player's stats
    ...
    tb.summary = { made: r.made, declarerTeam: dt, sweep: r.sweep, line1, line2, t: 0 };
    const w = tb.mode === 'lesson' ? -1 : matchWinner(tb.scores, tb.target, r);
    if (w >= 0) {
      tb.over = { winner: w, unlocked: 0 };
      // Auto Play never writes the real player's progress/best-level or the real save slot - it
      // reassigns the same `state.tb`/`scoreHand` a real match uses (the established, safe pattern
      // already used above for `state.stats.hands`), so nothing here can corrupt a real result.
      if (tb.mode !== 'auto') {
        state.stats.played++;
  ...
  function pressTable(x, y) {
    ...
    // Pre-existing bug fixed in passing (found while building Auto Play, which reaches this screen
    // every run): `BTN.again` was referenced here and in view.js but never defined in layout.js -
    // any match reaching its natural end would crash on the very next render. `BTN.next` is never
    // shown at the same time as this over-screen (it belongs to the hand-summary screen, which
  ...
  // Auto Play's own decision loop: THINK (board static, viewer works out their own guess -
  // configurable, THINK_STEPS, capped 10s) -> REVEAL (~2s: the pending bid/trump/card shown via
  // the same `tb.hint` mechanism the Hint button already uses, so every existing ring/glow/dim
  // rendering just works unchanged) -> ACT (doBid/doTrump/doPlay, the real action-execution path,
  // unchanged) -> loop, for every seat's every decision, for a whole match. The move-picker is
  // always the synchronous rule-of-thumb (ai.js chooseBid/chooseTrump/pickCard) rather than level
  // 4's multi-frame Monte-Carlo `createThinker` - same file, same real AI, just the always-ready
  // half of it, so THINK/REVEAL timing never has to wait on a still-computing search.
  function autoTick(dt) {
    const tb = state.tb, H = tb.H;
    if (tb.autoPhase === 'reveal') {
      tb.autoTimer -= dt;
      if (tb.autoTimer > 0) return;
      const ch = tb.autoChoice;
      tb.autoPhase = null; tb.autoChoice = null; tb.hint = null;
      if (ch.kind === 'bid') doBid(ch.p, ch.n);
      else if (ch.kind === 'trump') doTrump(ch.s);
      else doPlay(ch.c);
      return;
    }
    ...
    if (tb.autoPhase !== 'think') {
      let choice = null;
      if (H.phase === 'bid') { const p = H.bid.turn, b = chooseBid(H, p, tb.level, rng); choice = { kind: 'bid', p, n: b.n && canBid(H, b.n) ? b.n : 0 }; }
      else if (H.phase === 'trump') choice = { kind: 'trump', p: H.declarer, s: chooseTrump(H.hands[H.declarer]) };
      else if (H.phase === 'play') choice = { kind: 'play', p: H.turn, c: pickCard(H, H.turn, tb.level, rng).card };
      if (!choice) return; // hand already over; waiting for handEnd()'s own summary/over screen
      tb.autoChoice = choice; tb.autoPhase = 'think'; tb.autoTimer = THINK_STEPS[state.autoThinkIdx];
      say(`${SEAT_NAMES[choice.p]} is thinking…`, tb.autoTimer + AUTO_REVEAL_SECS + 1);
      return;
    }
    tb.autoTimer -= dt;
    if (tb.autoTimer > 0) return;
    const ch = tb.autoChoice, label = ch.kind === 'bid' ? 'bid' : ch.kind === 'trump' ? 'trump call' : 'card';
    tb.hint = ch.kind === 'bid' ? { n: ch.n } : ch.kind === 'trump' ? { s: ch.s } : { c: ch.c };
    tb.autoPhase = 'reveal'; tb.autoTimer = AUTO_REVEAL_SECS;
    say(`This is the ${label} - compare it with your own guess.`, AUTO_REVEAL_SECS + 0.5);
  }
  ...
  function tick(dt) {
    ...
    if (tb.summary) {
      // Auto Play reads each hand's summary (and, at the end of a match, the "See the result"
      // hand-off to the over-screen) at its own pace, then continues on its own - a real match
      // always waits for a tap; only this mode auto-advances. nextAfterSummary() already does the
      // right thing either way: starts the next hand, or - when tb.over is set - just clears the
      // summary and reveals the over-screen underneath it.
      if (tb.mode === 'auto') { tb.summary.t = (tb.summary.t || 0) + dt; if (tb.summary.t > 3.4) nextAfterSummary(); }
      return;
    }
    if (tb.flights.length || tb.over || tb.leaving || tb.blocked || puzzleOver(tb)) return;
    if (tb.mode === 'auto') { autoTick(dt); return; }
